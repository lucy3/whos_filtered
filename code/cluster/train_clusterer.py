        ...
        for json_file in tqdm(itertools.islice(self.files,
            ...
            for ix, item in enumerate(mapped_itr):
                if item[0] is not None:
                    idx = ix
                    if self.id_field in item[2]: 
                        idx = item[2][self.id_field]
                    res = {'id': idx, 'file': str(json_file), 'text': item[0], 'token_count': item[1]}
                    if self.path_to_clusters:
...
def vectorize_sklearn(model, files, batch_size=512, parallel=False, num_parallel_shards=10):
    dataset = IterableDomain(files=files)
    if parallel:
        import joblib
...
def get_files(data_dir):
    random.seed(0)
    print("Getting files...")
    files = [Path(y) for x in os.walk(data_dir) for y in glob(os.path.join(x[0], '*.json.gz'))]
    # random sample 25% of files with seed
    num_files = len(files) // 4
...
if __name__ == '__main__':
    '''
    Usage:
    python -m train_clusterer \
    --data-dir /net/nfs/allennlp/lucyl/cc_data/cc_bios_v1/ \
    --num-clusters 50 \
    --balanced \
    --output-dir /home/lucyl/llm_social_identities/outputs/kmeans/toy/${NUM_CLUSTERS}/
    '''
    ...
    path_to_terms = args.output_dir / "top_terms.csv"
    top_terms.to_csv(path_to_terms)  
